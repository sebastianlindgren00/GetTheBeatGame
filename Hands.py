...
class landmarker_and_result():
   ...
   def createLandmarker(self):
      ...
      options = mp.tasks.vision.HandLandmarkerOptions( 
         base_options = mp.tasks.BaseOptions(model_asset_path=os.path.join(path, "hand_landmarker.task")), # path to model
...
def main():
   ...
   while True:
      ...
      try:         
         ...
         if hand_landmarker.result.hand_landmarks != []:
            all_data = []
            da_time = str(time.time())

            for idx in range(len(hand_landmarker.result.hand_landmarks)):
               hand_landmarks = hand_landmarker.result.hand_landmarks[idx]
               output = {}
               data = [] 
               for landmark in hand_landmarks: 
                  data.append({"x": landmark.x, "y" : landmark.y, "z": landmark.z})
               handed = hand_landmarker.result.handedness[idx]
            ...
            #print(s)          
            f = open(os.path.join(path, "hands_data.txt"), "w")
            f.write(s)
            f.close()
