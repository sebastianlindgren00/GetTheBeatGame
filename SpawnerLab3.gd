...
var ballonRedPrefab = preload("res://Prefabs/balloonRed.tscn")
...
func spawn_balloon():
	...
	for i in floor(nr_of_balloons):
		...
		if balloonType == 0 and scene == "Lab 2": #Only spawn yellow balloons in lab 2 scene
			ballonPrefab = ballonYellowPrefab
			balloonColor = "YELLOW"
	
		var balloonInst = ballonPrefab.instantiate() # Instantiate prefab
	...
	nr_of_balloons += 0.3 # Every fourth pop add +1 to total spawned
